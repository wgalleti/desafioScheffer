...
const resolver = ({ values }) => {
  ...
  if (total === 0) {
    errors.rolinhos = [
      { message: "É necessário ao menos um rolinho ou fardão" },
    ];
  }
...
<template>
  ...
  >
    <div>
      ...
      />
      <Message
        v-if="$form.fazenda?.invalid"
        severity="error"
        size="small"
        variant="simple"
        >{{ $form.fazenda.error?.message }}</Message
      >
    </div>
    <div>
      ...
      />
      <Message
        v-if="$form.algodoeira?.invalid"
        severity="error"
        size="small"
        variant="simple"
        >{{ $form.algodoeira.error?.message }}</Message
      >
    </div>
    ...
    <div class="grid grid-cols-2 gap-2">
      <div>
        ...
        <InputNumber
          v-model="initialValues.rolinhos"
          name="rolinhos"
          :min="0"
          fluid
        />
      ...
      <div class="col-span-2 gap-2">
        <Message
          v-if="$form.rolinhos?.invalid"
          severity="error"
          size="small"
          variant="simple"
          >{{ $form.rolinhos.error?.message }}</Message
        >
      </div>
    </div>
